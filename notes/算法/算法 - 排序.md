...
|   算法   | 稳定性 |       平均时间复杂度       |     最好      |     最坏      | 空间复杂度 | 排序方式  |                 备注                 |
| :------: | :----: | :------------------------: | :-----------: | :-----------: | :--------: | :-------: | :----------------------------------: |
| 选择排序 |   ×    |       N<sup>2</sup>        | N<sup>2</sup> | N<sup>2</sup> |     1      | in palce  |                                      |
| 冒泡排序 |   √    |       N<sup>2</sup>        |       N       | N<sup>2</sup> |     1      | in palce  |        排序趟数和初始顺序有关        |
| 插入排序 |   √    |     N \~ N<sup>2</sup>     |       N       | N<sup>2</sup> |     1      | in palce  |       时间复杂度和初始顺序有关       |
| 希尔排序 |   ×    | N 的若干倍乘于递增序列长度 |               |               |     1      | in palce  | 改进版插入排序(复杂度和增量序列有关) |
| 快速排序 |   ×    |           NlogN            |     NlogN     | N<sup>2</sup> |    logN    | in palce  |        排序趟数和初始顺序有关        |
| 三路快排 |   ×    |         N \~ NlogN         |               |               |    logN    | in palce  |         适用于有大量重复主键         |
| 归并排序 |   √    |           NlogN            |     NlogN     |     NlogN     |     N      | out place |                                      |
|  堆排序  |   ×    |           NlogN            |     NlogN     |     NlogN     |     1      | in palce  |          无法利用局部性原理          |
| 计数排序 |   √    |            N+k             |      N+k      |      N+k      |     k      | out place |                                      |
|  桶排序  |   √    |            N+k             |      N+k      | N<sup>2</sup> |    N+k     | out place |                                      |
| 基数排序 |   √    |            N\*k            |     N\*k      |     N\*k      |    N+k     | out place |                                      |

快速排序是最快的通用排序算法，它的内循环的指令很少，而且它还能利用缓存，因为它总是顺序地访问数据。它的运行时间近似为 cNlogN，这里的 c 比其它线性对数级别的排序算法都要小。

使用三向切分快速排序(三路快排)，实际应用中可能出现的某些分布的输入能够达到线性级别，而其它排序算法仍然需要线性对数时间。
