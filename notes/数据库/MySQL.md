...
![](../../assets/cs-note/distribute/mk-2020-08-20-20-34-35.png)

redo log重做日志，主要的流程如上图。当得到数据操作的要求时候，从磁盘读取数据，然后在内存缓存中修改数据。并将操作行为记录进redo log buffer，并同时将数据写到磁盘。4、5两部是同时进行的。
